            ...
            {selectedMember && (
              <div>
                ...
                {restaurant!.hook_item && (
                  <div style={{ background: '#FFF9E6', border: '1.5px solid #F59E0B', borderRadius: 12, padding: '14px 16px', marginBottom: 12, textAlign: 'center' as const }}>
                    <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '.1em', color: '#92400E', marginBottom: 4 }}>COMPLIMENTARY ITEM</div>
                    <div style={{ fontSize: 18, fontWeight: 800, color: '#0A0F0D' }}>{restaurant!.hook_item}</div>
                    {restaurant!.hook_item_detail && <div style={{ fontSize: 11, color: '#92400E', marginTop: 2 }}>{restaurant!.hook_item_detail}</div>}
                  </div>
                )}
